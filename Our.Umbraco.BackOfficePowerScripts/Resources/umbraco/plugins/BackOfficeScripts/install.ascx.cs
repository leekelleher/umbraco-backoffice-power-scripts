...
// using uComponents.Core.Install.PackageActions;
using umbraco.cms.businesslogic.datatype;

namespace Our.Umbraco.BackOfficePowerScripts.Web
{
	/// <summary>
	/// 
	/// </summary>
	public partial class install : UserControl
	{
		...
		protected string Logo
		{
			get
			{
				return this.Page.ClientScript.GetWebResourceUrl(typeof(uComponentsInstaller), "uComponents.Core.Shared.Resources.Images.ucomponents_200x32.png");
			}
		}
			...
			foreach (ListItem item in this.cblUiModules.Items)
			{
				if (item.Selected)
				{
					try
					{
						...
						umbraco.cms.businesslogic.packager.PackageAction.RunPackageAction(item.Text, "uComponents_AddHttpModule", xml.FirstChild);
						successes.Add(item.Text);
					}
					catch (Exception ex)
					{
